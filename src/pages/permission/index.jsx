...
export default class Order extends React.Component{
        ...
        if (dataSource && dataSource.length > 0) {
            for (let i = 0; i < dataSource.length; i++) {
                const data = {
                    ...
                    title: dataSource[i].user_name,//固定结构
    ...
    handleUserSubmit = ()=>{
        ...
        data.role_id = this.state.selectedItem.id;//列表选中的项
        axios.ajax({
...
class PermEditForm extends React.Component {
    ...
    renderTreeNodes = (data) => {
        return data.map((item) => {
            if (item.children) {
                return (
                    <TreeNode title={item.title} key={item.key} dataRef={item} className="op-role-tree">
    ...
    render() {
        ...
        return (
            <Form layout="horizontal">
                <FormItem label="角色名称：" {...formItemLayout}>
                    <Input disabled maxLength={9} placeholder={detail_info.role_name}/>
                </FormItem>
                <FormItem label="状态：" {...formItemLayout}>
